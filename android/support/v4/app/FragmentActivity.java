  ...
  extends Activity
{
    ...
    if ("fragment".equals(paramString))
    {
      ...
      if ((localTypedArray2 != -1) || (localTypedArray1 != -1) || (str != null))
      {
        ...
        if (localFragment != null)
        {
          ...
          {
            throw new IllegalArgumentException(paramAttributeSet.getPositionDescription() + ": Duplicate id 0x" + Integer.toHexString(localTypedArray1) + ", tag " + str + ", or parent id 0x" + Integer.toHexString(localTypedArray2) + " with another fragment for " + (String)localObject);
          }
        }
        else
        {
          localFragment = Fragment.instantiate(this, (String)localObject);
        ...
        if (localFragment.mView != null)
        {
          ...
          if (localFragment.mView.getTag() == null) {
            localFragment.mView.setTag(str);
          }
          localObject = localFragment.mView;
        }
        else
        {
          throw new IllegalStateException("Fragment " + (String)localObject + " did not create a view.");
        }
      }
      else
      {
        throw new IllegalArgumentException(paramAttributeSet.getPositionDescription() + ": Must specify unique android:id, android:tag, or have a parent with an id for " + (String)localObject);
      }
    }
    else
    {
  ...
  public final Object onRetainNonConfigurationInstance()
  {
    ...
    for (int i = -1 + this.mAllLoaderManagers.size();; localNonConfigurationInstances--)
    {
      if (i < 0)
      {
      ...
      LoaderManagerImpl localLoaderManagerImpl = (LoaderManagerImpl)this.mAllLoaderManagers.valueAt(localNonConfigurationInstances);
      if (!localLoaderManagerImpl.mRetaining)
      {
        localLoaderManagerImpl.doDestroy();
        this.mAllLoaderManagers.removeAt(localNonConfigurationInstances);
      }
      else
      {
        j = 1;
